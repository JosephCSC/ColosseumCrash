...
public class ProtocolClient extends GameConnectionClient
{
	...
	private void createGhostNPC(Vector3f position) throws IOException
	{
		if(ghostNPC ==null)
		{
			...
			ghostNPC.getRenderStates().setModelOrientationCorrection((new Matrix4f()).rotationY((float)java.lang.Math.toRadians(-90.0f)));
			game.attackAnim();
		}
	}
	...
	private void updateGhostNPC(Vector3f position, double gsize)
	{
		...
		game.onTerrain(ghostNPC, 0);
		ghostNPC.lookAt(game.getAvatar());
		if (gsize == 1.0) gs=false; else gs=true;
		ghostNPC.setSize(gs);
	}
	
	public boolean isGhost(){return ghostNPC != null;}
	
	public Vector3f getGhostLocation(){
		return ghostNPC.getLocalLocation();
	}
	...
	{	String strMessage = (String)message;
		...
		if(messageTokens.length > 0)
		{
			...
			if (messageTokens[0].compareTo("move") == 0)
			{
				...
				ghostManager.updateGhostAvatarLoc(ghostID, ghostPosition);
			}
			...
			if (messageTokens[0].compareTo("rotate") == 0)
			{
				...
				Matrix4f ghostRot = new Matrix4f(
				...
				ghostManager.updateGhostAvatarRot(ghostID, ghostRot);
			}
			...
			//Handle GetLocation
			//format:(gaL)
			if(messageTokens[0].compareTo("gaL")==0)
			{
				Vector3f loc =game.getPlayerPosition();
				try 
				{	String message1  = new String("pLoc");
					message1 += "," + loc.x();
					message1 += "," + loc.y();
					message1 += "," + loc.z();
					sendPacket(message1);
			...
			// -----------Hendle NPC HIt--------------------------------------------
			if(messageTokens[0].compareTo("npcHit")==0)
			{
				game.NPCHit();
			}
		}
	...
	public void answerLocation()
	{
		try 
		{
			sendPacket(new String("location" + id.toString()));
		}
		catch (IOException e)
		{	e.printStackTrace();
		}
	...
	public void sendAddNPCMessage()
	{
		System.out.println("Message being sent");
		try
		{	sendPacket(new String("addNPC," + id.toString()));
		}
		catch (IOException e)
		{	e.printStackTrace();
		}
	...
	public void sendNPCDeadMessage(){
		try
		{	sendPacket(new String("deadNPC," + id.toString()));
		}
		catch (IOException e)
		{	e.printStackTrace();
		}
	...
	public void sendNPCHitMessage(){
		try
		{	sendPacket(new String("hitNPC," + id.toString()));
		}
		catch (IOException e)
		{	e.printStackTrace();
		}
