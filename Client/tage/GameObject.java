...
public class GameObject
{
	...
	*/
	...
	//Added Class to GameObject
	public void yaw(float y){
		Vector4f oldUp;
		Matrix4f rotAroundUp, oldRotation, newRotation;
		
		oldRotation = new Matrix4f(getWorldRotation());
		oldUp = new Vector4f(0f,1f,0f,1f).mul(oldRotation);
		rotAroundUp = new Matrix4f().rotation(y, new Vector3f(oldUp.x(), oldUp.y(), oldUp.z()));
	...
	public void pitch(float p){
		Vector4f oldForward;
		Matrix4f rotAroundForward, oldRotation, newRotation;
		
		oldRotation = new Matrix4f(getWorldRotation());
		oldForward = new Vector4f(1f,0f,0f,1f).mul(oldRotation);
		rotAroundForward = new Matrix4f().rotation(p, new Vector3f(oldForward.x(), oldForward.y(), oldForward.z()));
		newRotation = oldRotation;
		newRotation.mul(rotAroundForward);
		setLocalRotation(newRotation);
	}
	...
	public float getX(){
		Vector3f temp = new Vector3f(localTranslation.getTranslation(v));
		return temp.x();
	}
	...
	public void setPhysics(boolean p){
		hasPh = p;
	}
}