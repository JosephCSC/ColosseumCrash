...
public class GameServerUDP extends GameConnectionServer<UUID> 
{
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort)
	{
		...
		{	// JOIN -- Case where client just joined the server
			...
			//Reveived Message Format: (rotate, localID, 
			...
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				...
				sendRotateMessage(clientID, rot);
			}
			
			// -------------------Recieviving NPC information-----------------------
			...
			//revieved Meassage Format: (pLoc)
			if(messageTokens[0].compareTo("pLoc") == 0)
			{
				double x = Double.parseDouble(messageTokens[1]);
				double y = Double.parseDouble(messageTokens[2]);
				double z = Double.parseDouble(messageTokens[3]);
				npcCtrl.setAvX(x);
				npcCtrl.setAvY(y);
				npcCtrl.setAvZ(z);
				npcCtrl.setJoined(true);
				System.out.println(x);
			}
			
			//receaved request to add npc
			if(messageTokens[0].compareTo("addNPC") == 0)
			{
			...
			//recieve NPC hit
			if(messageTokens[0].compareTo("hitNPC")== 0)
			{
				sendNPCHitMessage();
			}
		}	
	...
	public void sendRotateMessage(UUID clientID, String[] rotation){
		try{
	...
	public void sendGetAvatarLoc()
	{
		try 
		{	String message = new String("gaL");
			sendPacketToAll(message);
		} 
		catch (IOException e) 
		{	System.out.println("couldnt send gaL message");
				e.printStackTrace();
		}
	}
	...
	{
		try 
		{	String message = new String("npcHit");
			sendPacketToAll(message);
		} 
		catch (IOException e) 
		{	System.out.println("couldnt send NPC hit message");
				e.printStackTrace();
		}
	}
